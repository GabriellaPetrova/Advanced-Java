...
public class Robotics {
    ...
    private static LocalDateTime getLocalDateTime(HashMap<String, Robot> allRobots, DateTimeFormatter formatter,
            LocalDateTime starTime, ArrayDeque<String> products, String product) {
        starTime = starTime.plusSeconds(1);

        LocalDateTime finalStarTime = starTime;
        Optional<Map.Entry<String, Robot>> tempRobot = allRobots.entrySet().stream()
                .filter(s -> s.getValue().endTime
                .compareTo(finalStarTime) <= 0).findFirst();
